...
% % 
    ...
    alpha = 1:3:180; % AoD，可调整
    tau = (0 * 10^-9):(1 * 10^-9):(20 * 10^-9);%ToF adjustable
    Pmusic4D = zeros(length(theta), length(phi), length(tau), length(alpha));
...
for ii = 1:length(theta)
    for kk = 1:length(phi)
        for jj = 1:length(tau) 
            for hh = 1:length(alpha) % AoD循环
                steering_vector = compute_steering_vector_4D(theta(ii), phi(kk), tau(jj), alpha(hh), ...
                        frequency, sub_freq_delta, antenna_distance);
                PP = steering_vector' * eigenvectors * eigenvectors' * steering_vector;
                Pmusic4D(ii, kk, jj, hh) = abs(1 / PP);
            end
        end
    end
end
    ...
    PPstatic = squeeze(sum(sum(Pmusic4D, 4), 3));
