...
csi_trace = read_bf_file(fullfile('my_data', 'lb_01_01_01.dat')); %data_1DAoA.dat
...
sample_csi_trace = sample_csi_traceTmp.csi_out;
sample_csi_trace =squeeze(sample_csi_trace(1,:,:,:));
sample_csi_trace = reshape(sample_csi_trace,90,20);
...
k=1;
 ...
 for i=1:size0
     ...
     csi = sample_csi_trace(:,i);
     
    antenna1_card1(k,:)=csi(1,:);
    antenna2_card1(k,:)=csi(2,:);
    antenna3_card1(k,:)=csi(3,:);

    antenna1_card1(k,:)=csi(1:30,:);
    antenna2_card1(k,:)=csi(31:60,:);
    antenna3_card1(k,:)=csi(61:90,:);
...
%     
        % Theta (AoA) & Tau (ToF) 3D Plot
        I = figure('Name', 'AoA & ToF MUSIC Peaks', 'NumberTitle', 'off');
