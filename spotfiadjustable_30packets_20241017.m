...
process_dat_files('E:\WorkSpace\datasets\xrf55\CSI', 'E:\WorkSpace\datasets\xrf55\AOA-TOF')

function process_dat_files(sourceFolder, targetFolder)
    ...
    for i = 1:length(files)
        ...
        try
            ...
            estimate_aoa_tof(sourceFilePath, targetFilePath, false);
            % 输出结果并记录日志
            fprintf('结果已保存到: %s\n', targetFilePath);
...
function [tau, theta, Pmusic] = estimate_aoa_tof(source_path, save_path, visualize)
    % 参数设置
    SignalEndIdx = 25
    ...
    antenna1_card1 = zeros(size0, 30);
    antenna2_card1 = zeros(size0, 30);
    antenna3_card1 = zeros(size0, 30);
